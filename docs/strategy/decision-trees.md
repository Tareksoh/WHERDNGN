...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| **Sun contract**, your team's cumulative score is ≥100 | **Bel is FORBIDDEN** — legality gate, not heuristic. | Hard Saudi rule: only the team <100 may Bel in Sun. Hokm has no such gate. | **wired**: `R.CanBel` (Rules.lua:637-675) is the legality gate; `Bot.PickDouble` consults it (Bot.lua:6045-6046); UI / Net call paths also check (UI.lua:1823-1824, Net.lua:1070-1071). Tests: test_rules.lua:1161-1237, test_state_bot.lua:985-1015. | Definite | 11 |
...
| Round 1 of any session | **Round-1 Bel restricted** (exact rule TBD from follow-up video) | Anti-grief convention | Need follow-up video to confirm exact mechanism. **DEFERRED — pending video research.** | Sometimes | 11 |
...
| Hokm contract, your team is the **bidder** team, you've won tricks 1-2 cleanly without opponent cut, hand has Kaboot-feasible shape (J+9 trump + cover + void/singleton + partner-supply) | Switch to **Al-Kaboot pursuit mode** at trick 3 (not trick 8) | Sweep needs the full 8 — only viable if no opp cut surfaces in tricks 1-2. Earlier-trigger pursuit lets you optimize tricks 3-7 for sweep. | **wired** v0.5.19 + v1.0.3: `sweepPursuitEarly` fires from trick 3 with M3lm hand-shape feasibility gate (Bot.lua:2491-2547). Doc previously said "Partial wire" (stale). | Common | 15 |
...
| Bidder, Sun contract, you're winning + score-tracking shows `K.MULT_BEL × hand_total > K.AL_KABOOT_SUN` | **Sabotage own sweep** (تخريب الكبوت) — let opp win one trick to land at multiplier instead | Multiplier path scores higher than the sweep when Bel was called — IF Kaboot bonus is NOT multiplied. | **DOES NOT APPLY** in WHEREDNGN scoring rules. v1.4.1 user-clarified: this rule assumes the Kaboot bonus is FLAT (un-multiplied). Per Rules.lua:1273-1275, regular Al-Kaboot IS multiplied by `cardMult` (Bel×2 → Al-Kaboot bonus also ×2). Only Reverse Al-Kaboot bypasses the multiplier (`K.MULT_BASE`). With the bonus multiplied, sweep always dominates the multiplier-only path; sabotage rule is irrelevant. **Closed — no implementation needed.** | Sometimes | 15 |
...
| **Reverse Al-Kaboot** — defenders sweep all 8 against bidder; bidder was trick-1 leader | Score +88 banta (final score) to defending team. Per Saudi convention this bonus bypasses the contract multiplier. | Saudi rule: full defender sweep is a recognized scoring outcome with its own bonus. | **wired** v0.10.5 → v1.0.12 (4-condition canonical gate: Sun + dealer-right + bidder-had-Ace). `K.AL_KABOOT_REVERSE = 880` (Constants.lua:170 — note: 880 is the post-multiplier raw value yielding +88 banta because `cardMult` is bypassed; doc previously said "+88 raw" which was a unit error — code is correct). `R.ScoreRound` Rules.lua:960-1023; tests test_rules.lua:610-680. | Common | 16 |
...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| Sun, partner winning trick; you must discard; you hold A of suit X and want partner to lead X | **Bargiya** — discard the A of X. Strongest "want this suit" signal. | Ace-discard = "I have the slam in X, lead it." | **wired** v0.9.0 + v1.0.3 + v1.2.1 G7: Bot.lua:4297-4347 (Sun-only, A+cover late-game / cornered exception early-game). | Definite | 01, 03 |
| Sun, partner winning; you want suit X (without holding A of X) | **Bottom-up sequence in X**: discard low first (7/8), higher (9/J) on next discard. | Two-trick ascending sequence = "I want this suit, no Ace to Bargiya." | **wired** v0.9.0 (Bot.lua:4373-4396), but ⚠ **v1.4.0 audit-flagged contradiction**: current code emits bottom-up from a suit WITH A or T (≥3 cards), but video #3 says signal should come from a WEAK suit. See code comment at Bot.lua:4373. DEFERRED reversal pending cross-video reconciliation. | Definite | 01, 09, 10 |
| Sun, partner winning; you do NOT want suit X | **Top-down sequence in X**: discard high first (J/9), lower next. | Two-trick descending = "I refuse this suit." | **partially wired** via T-4 dump-ordering (Bot.lua:4416-4432) for 2-card case; multi-event descending sequence not explicitly tracked but T-4 covers the most common case. | Definite | 01, 10 |
| Sun, partner winning; you must dump from a 2-card suit you don't want (e.g. J + 9 of unwanted suit) | Always dump the **LARGER first** (J before 9), never smaller first. | Larger first = unambiguous refusal; smaller first = false bottom-up positive signal. | **wired** v0.5.10 + v0.5.11 (Bot.lua:4416-4432) with K/T/A high-card preservation gate. | Definite | 01 |
...
| Receiver: you have NO winning card in the Tahreeb-suggested suit on second event | Send your **highest available** card back to partner — the high return is your only contribution. Do NOT play absolute lowest. | Even forced, cooperate maximally. Lowest = "biggest mistake in Baloot". | `pickFollow` high-card-return discipline `(not yet wired)`. **DEFERRED v1.4.1** — linked to Concern 1 (Tahreeb sender semantics). The "Tahreeb-suggested suit" identification depends on receiver-side decoding of sender's signal forms (Bargiya vs bottom-up vs top-down). v1.4.1 added deep code-comment analysis at `Bot.lua:4373` documenting the sender-side complexity per videos 1, 3, 5. The receiver-side high-card discipline can be implemented after the sender semantics are reconciled. | Definite | 09 |
...
| Receiver: you've already won partner's Tahreeb-return trick; partner re-supplies the suit | **Do NOT capture again with T** (release control). | Holding the T blocks partner's continuation; partner may have K or J to win the next trick. | `pickFollow` release-control branch `(not yet wired)`. **DEFERRED v1.4.1** — linked to Concern 1. Release-control logic requires the receiver to have correctly decoded partner's Tahreeb signal type to identify the "Tahreeb-suggested suit" — same dependency as row 238 above. Implement after sender semantics reconciled. | Common | 02 |
...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| OPPONENT (not partner) has 100% taken the trick; you must discard / ruff | Discard the suit you DO want partner to return (inverse of Tahreeb). | Against opp there's no point hiding strength via "negative discard"; positive-signal directly. | `pickFollow` N-1 sender at Bot.lua (search "v0.5.14 Section 9 N-1") — **wired v0.5.14**. | Common | 03 |
| Trick-winner uncertain (not near-certain who wins) | **Default to Tahreeb semantics, NOT Tanfeer.** Tahreeb is the dominant convention. | Speaker explicit: "تهريب اقوى من تنفير وقاعده تهريب تمشي معاك اكثر." | `pickFollow` discard branch fallback — **wired v0.5.14** (Tahreeb sender block runs by default; N-2 documented in code comments). | Common | 03 |
| Opponent does small→big tahreeb-style discard on you | Recognize: opp wants that suit returned by their partner. **Do NOT lead that suit; treat as suit-to-AVOID.** | Same convention applies to opp; useful when opp's partner has cards there. | `pickLead` N-3 receiver — **wired v0.5.14** at Bot.lua tahreebAvoidSet block (search "v0.5.14 Section 9 N-3"). | Common | 10 |
...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| **Hokm**, no exception condition active | **Default: do NOT Faranka.** Play natural high trump. | Trump is short; withholding high-variance and dangerous. | `pickFollow` natural winners-branch (cheapest winner / lowest trump) — **wired by absence**: bot has no Faranka path unless an exception fires. | Definite | 04 |
| Hokm exception #1: pursuing Al-Kaboot ("النوع الثالث") | Faranka allowed — variance acceptable because losing ANY trick already kills Kaboot. | "تقريبا نفس الصمت" — approximates Sun-style silence. | `pickLead` trick-3 sweep-pursuit branch (Bot.lua ~1544) — **partially wired v0.5.19** for sweep-track detection; cross-wire to pickFollow Faranka still deferred. | Common | 04 |
| Hokm exception #2: you hold only 2 trumps total + on bidder team | Faranka allowed — trump posture already weak. | Costs little incremental EV. | `pickFollow` two-trump Faranka branch — **wired v0.8.4**, bidder-team gate added v0.9.2 (audit #49). | Common | 04 |
| Hokm exception #3: J of trump already played/dead, your 9 is now top live trump | Faranka allowed — withhold the new top to ambush opp's other high. | Your 9 is now the boss. | `pickFollow` top-trump-shifted branch — **wired v0.8.5** via `S.HighestUnplayedRank(trump) == "9"` predicate (which was itself fixed in v0.8.5 to use TRUMP_HOKM_ORDER). | Common | 04 |
| Hokm exception #4: you are bidder + opponent trump exhausted | Faranka allowed — no one can punish the withhold. | Risk-free at this point. | `pickFollow` bidder-clean-trump branch — **wired v0.8.4** via `Bot._memory[s].void[trump]` per-opp check. | Common | 04 |
...
| Hokm; bidder is opp; opp led trump-Q; you hold trump-J + trump-8 | **Do NOT Faranka — play J normally, take trick.** | Opp has long trump; withholding gets rolled later. Direct counter-rule. | `pickFollow` J+8 anti-Faranka rebuttal — **wired v0.8.4**. Overrides exceptions #2 / #3 / #4 when opp-bidder Q-led + J+8 in hand. | Definite | 04 |
...
| Hokm trump count: 5 visible, 3 remain, you and partner hold 0 of remaining | Pigeonhole: **all 3 remaining trumps in the same opponent's hand** — pin them in sampler. | Mathematical force. Extension of H-1 J/9-pin. | `BotMaster.PickPlay` sampler — **wired v0.5.22** at the H-1 pin extension block (search "Rule 3 pigeonhole pin"). | Definite | 05 |
| Partner is Sun bidder | Assume partner has **one long suit** (4+) with concentrated high cards. | Sun bid pattern; mixed-strength hands bid Hokm or pass. | Sampler bias — **wired v0.6.1** in `getPartnerCards` Sun branch (`desire["A"..s] = 8`, `desire["K"..s] = 4`). | Common | 02 |
| Opp just performed `pickFollow.deceptiveOverplay` (sacrificed top) | Update ledger — they've shown the bait once; bait against same opp this round less likely to land. | "Bait detected" tracking. | `Bot._partnerStyle[seat].baitedSuit[suit]` ledger — **wired v0.8.2**. Read by `pickLead` defender branch as avoid-suit hint. | Sometimes | 08 |
