...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| **Sun**, you are losing-side off-suit follow (can't beat winning card, must follow suit) | Dump the **HIGHEST** spare you can — never the lowest. | Saudi inverse-laddering convention: under a higher card you've lost to, dump second-highest, signal partner. Leading absolute lowest = "biggest mistake in Baloot". | `pickFollow` Bot.lua:1457 losing-side discard branch `(not yet wired)`. | Definite | 05, 09 |
...
| WHEN | RULE | WHY | MAPS-TO | CONFIDENCE | SOURCES |
|---|---|---|---|---|---|
| Sun, opp plays K or higher in 2nd-position when next-to-act, under your higher card | Infer: opp has **NO card lower than the played rank** in that suit (~95% confidence at K, ~90% at Q). | Saudi losing-side dump-highest convention. | Sampler hand-reconstruction constraint `(not yet wired)`. New key suggestion: `Bot._partnerStyle[seat].dumpHighSeen[suit]`. | Common | 05 |
...
| Sun off-suit losing-side dump direction | Dump HIGHEST | Dump LOWEST | Sun rule = HIGHEST. Hokm trump-follow rule = LOWEST. Not a contradiction, contract-conditioned. Both rows present in Section 4. |
