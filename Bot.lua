...
-- Bidding-threshold aliases (TH_HOKM_R1_BASE / TH_HOKM_R2_BASE /
-- TH_SUN_BASE / BID_JITTER) plus the multi-version calibration
-- rationale comment moved to Bot/Bidding.lua in v3.2.0 cleanup
-- batch 8 (all call sites are inside the moved Bot.PickBid).

-- Tier-detection predicates (Bot.IsAdvanced / IsM3lm / IsFzloky /
-- IsSaudiMaster / IsBotSeat) live in Bot/Tiers.lua, which the .toc
-- loads before this file. The five functions are set on the shared
-- B.Bot table so every call site below — `Bot.IsAdvanced()`,
-- `Bot.IsM3lm()`, etc. — resolves through the same `local Bot =
-- B.Bot` reference at runtime without needing a re-binding header.
...
-- PickAKA / escalation deciders / etc.) resolves unchanged. COL
-- mirror pattern: the inter-primitive `pickRandomTied` call inside
-- the new module uses its own file-local closure — the re-bound
-- local here is for Bot.lua's consumers only.
...
-- Batch-8 Bidding re-binding header (sunStrength / suitStrengthAsTrump /
-- partnerBidBonus / partnerEscalatedBonus / combinedUrgency /
-- opponentUrgency) removed in v3.2.0 cleanup batch 9: the escalation
-- deciders that consumed them moved to Bot/Escalation.lua, and no
-- other Bot.lua code path uses these Bidding helpers post-extraction.
...
-- Convenience derived metric for play-side decisions. Returns nil if
-- we haven't seen enough plays to call a trump-tempo tendency; falls
-- through to neutral default in that case.
--
-- Consumers: pickLead / pickFollow trump-pull heuristics (Bot.lua
-- lines ~2061-2102 and ~2295-2296). The companion `styleBelTendency`
-- helper (also derived from Bot._partnerStyle) moved to
-- Bot/Escalation.lua in v3.2.0 cleanup batch 9 because its only
-- caller is Bot.PickTriple.
...
-- Bidding helpers
-- ---------------------------------------------------------------------
-- sideSuitAceBonus, hokmMinShape, sunMinShape, beloteSuit,
-- beloteBypassQualifies, aceCountAndMardoofa moved to Bot/Bidding.lua
-- in v3.2.0 cleanup batch 8 (along with Bot._beloteBypassQualifies
-- test-internal export). The bidderHoldsBidcard helper below STAYS in
-- this file because its only caller is pickLead/pickFollow trump-J
-- inference.
...
-- Play-primitive helpers (pickRandomTied / lowestByRank / highestByRank
-- / highestByFaceValue / holdsBeloteThusFar / highestTrump /
-- legalPlaysFor / wouldWin / tahreebClassify / applyClosedTrumpLeadGate)
-- moved to Bot/PlayPrimitives.lua in v3.2.0 cleanup batch 5C. Their
-- file-local re-bindings live near the top of this file (just below
-- the Tier-detection breadcrumb) so pickLead / pickFollow and the
-- escalation deciders below close over the bound locals unchanged.
...
-- Escalation
-- ---------------------------------------------------------------------
-- The four-rung escalation chain (Bel x2 -> Triple x3 -> Four x4 ->
-- Gahwa match-win) plus its 3 file-local helpers (escalationStrength,
-- selfStyleJitterBonus, styleBelTendency) and 4 per-rung jitter
-- constants (BEL_JITTER, TRIPLE_JITTER, FOUR_JITTER, GAHWA_JITTER)
-- moved to Bot/Escalation.lua in v3.2.0 cleanup batch 9. The pickers
-- (Bot.PickDouble / PickTriple / PickFour / PickGahwa) remain public
-- on the shared B.Bot.* table; Net.lua resolves them at runtime. The
-- style-ledger maintenance code (Bot.OnEscalation, Bot.OnRoundEnd,
-- emptyStyle, styleTrumpTempo, Bot._partnerStyle, Bot._memory) STAYS
-- in this file because it is consumed by State.lua and pickLead/
-- pickFollow.
...
-- Bot.PickBid). They remain public on the shared B.Bot.* table; no
-- re-binding in this file is needed because Bot.lua has no remaining
-- internal call sites for either function.
