...
do
    ...
    -- saudi-rules.md:26-28 — first non-pass wins; non-bidder cross-
    -- trump take never existed in canonical play). The decision string
    -- is now silently rejected; ResolveOvercall returns not-taken.

    -- Single non-bidder TAKE_HOKM with valid different suit → rejected.
    res = R.ResolveOvercall(
        { [3] = "TAKE_HOKM_S" },
        hokm, "9C", 4)  -- contract trump = C; bidder=1; dealer=4
    assertEq(res.taken, false,               "P.23: TAKE_HOKM_S no longer accepted (non-canonical)")

    -- TAKE_HOKM_<contract.trump> (same as current trump) → still rejected.
    res = R.ResolveOvercall(
        { [3] = "TAKE_HOKM_C" },  -- same as bidder's trump C
        hokm, "9C", 4)
    assertEq(res.taken, false,               "P.24: TAKE_HOKM same-as-current-trump rejected")

    -- TAKE_HOKM with malformed suit → still rejected silently.
    res = R.ResolveOvercall(
        { [3] = "TAKE_HOKM_X" },
        hokm, "9C", 4)
    assertEq(res.taken, false,               "P.25: TAKE_HOKM_<invalid-suit> rejected")

    -- Bidder UPGRADE wins over any TAKE_HOKM (whether accepted or not).
    res = R.ResolveOvercall(
        { [1] = "UPGRADE", [3] = "TAKE_HOKM_S" },
        hokm, "9C", 4)
    assertEq(res.type, "UPGRADE",            "P.26: bidder UPGRADE still resolves; TAKE_HOKM ignored")

    -- TAKE_HOKM in mixed decisions: drops to WAIVE; only TAKE counts.
    -- dealer=4, order = 1,2,3,4. Seat 1 is bidder. TAKE_HOKM at 2 is
    -- ignored; TAKE at 3 becomes the winning decision.
    res = R.ResolveOvercall(
        { [2] = "TAKE_HOKM_H", [3] = "TAKE" },
        hokm, "9C", 4)
    assertEq(res.by,   3,                    "P.27: TAKE_HOKM at seat 2 ignored; TAKE at seat 3 wins")
    assertEq(res.type, "TAKE",               "P.27: type=TAKE (Sun)")

    -- Earlier-seat TAKE still wins; TAKE_HOKM at later seat is moot.
    res = R.ResolveOvercall(
    ...
    -- Forced contract → no overcall (unchanged).
    res = R.ResolveOvercall(
        { [2] = "TAKE_HOKM_S" },
        takweesh, "9C", 4)
    assertEq(res.taken, false,               "P.29: TAKE_HOKM blocked on forced contract")

    -- v1.5.3 NEW: only-TAKE_HOKM decisions across all seats → not-taken.
    res = R.ResolveOvercall(
        { [2] = "TAKE_HOKM_H", [3] = "TAKE_HOKM_S", [4] = "TAKE_HOKM_D" },
        hokm, "9C", 4)
    assertEq(res.taken, false,               "P.30 (v1.5.3): all TAKE_HOKM → no overcall (Hokm stands)")
end
